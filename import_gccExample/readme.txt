TNRS Simplified Darwin Core format & TNRS database loading instructions
...
This document describes the TNRS Simplified Darwin Core (sDwC) format, the recommended 
export format for taxonomic data that will be used as a taxonomic source for the TNRS  
(Taxonomic Name Resolution Service, http://tnrs.iplantcollaborative.org/). For those 
building their own instance of the TNRS, it also provides a brief overview of how to 
build a TNRS reference database using sDwC taxonomic sources.
...
version of DwC have been added; these are indicated by an asterisk (*). This format 
supports most types of taxonomic names with the exception of quadrinomials (for example,
Ptelea trifoliata ssp. pallida var. cognata). 

File type and character encoding

If you will be providing your taxonomic data as a stand-alone file (as opposed to a 
database table via server connection), please save the file as plain-text utf8 to 
preserve all accented characters. Avoid opening or editing the file in Excel; this 
usually results in corruption of utf8 or extended ascii characters. The text file 
can be either tab-delimited or comma-delimited (.csv).

Taxonomic file schema (name, order and content of the data columns)

To use this import template, you taxonomic source data must match the format of the 
example file "dwcExample.csv" in the directory data/. It must have the same columns
listed below, in the same order as listed. If your taxonomic source contains only 
concatenated names, use the TNRS in parse-only mode to break the names into their 
components (http://tnrs.iplantcollaborative.org/TNRSapp.html).

taxonID				Required. Unique identifier for each taxon (name), preferably the 
					original primary key from the source database. Can be character or 
					numeric. MUST be unique for each record. 
parentNameUsageID	Link to identifier (taxonID) of most proximate parent taxon. Required
					for any taxon that has a parent. Although the scripts that build the 
					TNRS database will link orphan parentNameUsageIDs (linking to nonexistent 
					taxonIDs) will be linked to the root, this is undesireable as the 
					position of these names in the taxonomic hierarchy will be lost, and 
					the names will not be available for fuzzy matching, only exact 
					matching. WARNING: circular parentNameUsageID-taxonID links will cause 
					database loading to fail.
...
scientificNameAuthorship	Optional but recommended. This is the terminal authority of 
					the taxon. Please do not include any internal authorities (for 
					example, if the name is a subspecies, this should be the authority for 
					the subspecies, not the species. Required if your list contains 
					homonyms (duplicate names  disambiguated only by authority).
...
speciesHybridMarker	*Optional but recommended. Leave blank if not a hybrid name. The TNRS 
					loading scripts will detect most hybrids.
specificEpithet		Required if taxon at rank of species or lower. Just the epithet; do 
					not include the genus portion of a species name. Leave blank for 
					hybrid formula names (e.g., Adiantum latifolium x petiolatum).
...
infraspecificRank2	*Required if name is an infraspecific quadrinomial (e.g., 
					"Silene laciniata ssp. major var. angustifolia"). These will be taxa 
					of ranks variety, forma, subforma, etc. Not directly 
					equivalent to any Darwin Core term. Controlled vocabulary as per 
					abbreviations as used in the ICBN Vienna code 
					(http://ibot.sav.sk/icbn/main.htm; see esp. Section 5): 
...
taxonomicStatus		Indications of simple taxonomic status. If name is a synonym, more 
					detailed reason for (nomenclaturally) invalid or illegitimate names 
					can be used. If left NULL, TNRS will treat name as "No opinion". 
					Current controlled vocabulary supported: 
...
How to build the TNRS database using sDwC taxonomic sources
...
5. Edit params.inc, making the following mandatory changes:
	...
	(c) $filepath="import_[short source name]/data/". For example:
		$filepath="import_dwcExample/data/";
	(d) $namesfile = "[taxonomic source file name]"; Use the name and extension of the 
		taxonomic data file you placed in folder data/.
	(e) Make sure the "MySQL LOAD DATA INFILE parameters" are appropriate for the format 	
		of  your file (for more information, see 
6. If you are import other taxonomic source in darwin core format, prepare a separate 
	import directory for each one.		
7. Edit global_params.inc, making the following changes:
	(a) Add your source name(s) to the array $src_array. For example,
		...
		The source names in array $src_array (in global_params.inc) MUST match the 
		values of $sourceName in params.inc for each source AND the suffix of each 
		import directory.
